...
void loop()
{
	int32_t contributions = getContributions(github_username);
	...
	M5.Lcd.drawCentreString(String(contributions), 160, 65, 2);
...
int32_t getContributions(String username)
{
	...
	HTTPClient http;
	http.begin(apiServer + username + "/count");
	...
	if (httpCode > 0)
	{
		...
		return contributions_count.toInt();
	}
	else
	{
		http.end();
